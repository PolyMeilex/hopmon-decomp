...
class CD3DFramework7
{
    ...
    HWND                 m_hWnd;               // 0x00, the window object
    BOOL                 m_bIsFullscreen;      // 0x04, fullscreen vs. windowed
    BOOL                 m_bIsStereo;          // 0x08, stereo view mode
    DWORD                m_dwRenderWidth;      // 0x0C, dimensions of the render target
    DWORD                m_dwRenderHeight;     // 0x10
    RECT                 m_rcScreenRect;       // 0x14, screen rect for window
    LPDIRECTDRAW7        m_pDD;                // 0x24, the DirectDraw object
    LPDIRECT3D7          m_pD3D;               // 0x28, the Direct3D object
    LPDIRECT3DDEVICE7    m_pd3dDevice;         // 0x2C, the D3D device
    LPDIRECTDRAWSURFACE7 m_pddsFrontBuffer;    // 0x30, the primary surface
    LPDIRECTDRAWSURFACE7 m_pddsBackBuffer;     // 0x34, the backbuffer surface
    LPDIRECTDRAWSURFACE7 m_pddsBackBufferLeft; // 0x38, for stereo modes
    LPDIRECTDRAWSURFACE7 m_pddsZBuffer;        // 0x3C, the zbuffer surface
    DWORD                m_dwDeviceMemType;    // 0x40
    ...
    HRESULT CreateClipperForBackBuffer();
...
public:
    ...
    DWORD   GetRenderWidth() const;
    DWORD   GetRenderHeight() const;
};
ASSERT_SIZE(CD3DFramework7, 0x44);
